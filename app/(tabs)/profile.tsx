      ...
      {profile && levelInfo.nextTitle && (
        <View style={styles.progressSection}>
          <View style={styles.progressLabelRow}>
            <Text style={styles.progressLabel}>Keyingi daraja: {levelInfo.nextTitle}</Text>
        ...
        <View style={styles.achievementsGrid}>
          {achievements.map((a) => (
            <View key={a.id} style={[styles.achieveCard, !a.unlocked && styles.achieveCardLocked]}>
              <Text style={[styles.achieveIcon, !a.unlocked && styles.achieveIconLocked]}>{a.unlocked ? a.icon : "🔒"}</Text>
              ...
              return (
                <View key={entry.user_id} style={[styles.leaderRow, isMe && styles.leaderRowMe, i < leaderboard.length - 1 && styles.leaderBorder]}>
                  ...
                  <Text style={[styles.leaderName, isMe && styles.leaderNameMe]} numberOfLines={1}>
                    {entry.full_name || (isMe ? user.email?.split("@")[0] : `Foydalanuvchi`)}
                  </Text>
...
const styles = StyleSheet.create({
  ...
  emailText: { fontFamily: "Nunito_700Bold", fontSize: 14, color: C.text },
