...
const LAYERS = [
  ...
  { id: "machines",   label: "Avtomatlar", color: "#B45309" },
];
...
type SelectedItem = {
  type: "report" | "recycling" | "bin" | "machine";
  data: any;
  distance?: number;
};
...
export default function MapScreen() {
  ...
  const bottomPad = insets.bottom + 84 + 16;
  ...
  const handleMarkerPress = (type: SelectedItem["type"], data: any) => {
    const distance = userLocation
      ? haversineDistance(userLocation.latitude, userLocation.longitude, data.lat, data.lng)
      : undefined;
  ...
  const handleVerify = async () => {
    ...
    const pointTypeMap: Record<SelectedItem["type"], "report" | "recycling_point" | "waste_bin" | "reverse_vending_machine"> = {
      report: "report",
      recycling: "recycling_point",
      bin: "waste_bin",
      machine: "reverse_vending_machine",
    };

    setVerifying(true);
    const { error } = await verifyPoint(pointTypeMap[selectedItem.type], selectedItem.data.id, user.id);
    setVerifying(false);
  ...
  const getRecyclingBadgeColor = (type: string) => {
    const map: Record<string, { bg: string; text: string; label: string }> = {
      paper:    { bg: "#DBEAFE", text: "#1D4ED8", label: "Qog'oz" },
      plastic:  { bg: "#EDE9FE", text: "#7C3AED", label: "Plastik" },
      mixed:    { bg: "#DCFCE7", text: "#16A34A", label: "Aralash" },
      glass:    { bg: "#FEF3C7", text: "#D97706", label: "Shisha" },
      hazardous:{ bg: "#F3F4F6", text: "#374151", label: "Zararli" },
    };
  ...
  const visibleMachines  = activeLayers.has("machines") ? machines          : [];
      ...
      >
        ...
        {visibleMachines.map(machine => (
          <Marker
            ...
            pinColor={getMarkerColor("machines")}
            onPress={() => handleMarkerPress("machine", machine)}
          />
        ))}
      </MapView>
            ...
            <>
              <View style={styles.popupBadgeRow}>
                ...
                {selectedItem.type === "recycling" && (() => {
                  const cfg = getRecyclingBadgeColor(selectedItem.data.type);
                  return (
                ...
                {selectedItem.type === "bin" && (
                  <View style={[styles.badge, { backgroundColor: "#CFFAFE" }]}>
                    <Text style={[styles.badgeText, { color: "#0891B2" }]}>
                      {getBinLabel(selectedItem.data.bin_type)}
                    </Text>
                  </View>
                )}
                {selectedItem.type === "machine" && (
                  <View style={[styles.badge, { backgroundColor: "#FEF3C7" }]}>
                    <MaterialCommunityIcons name="robot-outline" size={12} color="#B45309" />
                    <Text style={[styles.badgeText, { color: "#B45309", marginLeft: 4 }]}>Avtomat</Text>
                  </View>
                )}
                {selectedItem.distance !== undefined && (
                  <View style={[styles.badge, { backgroundColor: "#F3F4F6" }]}>
                    <Text style={[styles.badgeText, { color: "#6B7280" }]}>
                      📍 {formatDistance(selectedItem.distance)} uzoqda
                    </Text>
                  </View>
                )}
              </View>
              ...
              {selectedItem.type === "report" && (
                <>
                  ...
                  {(selectedItem.data.verification_count ?? 0) > 0 && (
                    <View style={[styles.confirmRow, { backgroundColor: "#EDE9FE", marginTop: 4 }]}>
                      <Ionicons name="shield-checkmark" size={16} color="#7C3AED" />
                      <Text style={[styles.confirmText, { color: "#7C3AED" }]}>
                        {selectedItem.data.verification_count} marta tasdiqlangan
                      </Text>
                    </View>
                  )}
                </>
                ...
                <>
                  ...
                  {(selectedItem.data.verification_count ?? 0) > 0 && (
                    <View style={[styles.confirmRow, { marginTop: 4 }]}>
                      ...
                      <Text style={styles.confirmText}>{selectedItem.data.verification_count}x tasdiqlangan</Text>
                    </View>
                  )}
                </>
              )}
                ...
                <>
                  ...
                  {selectedItem.data.operator && (
                    <View style={styles.addressRow}>
                      ...
                      <Text style={styles.popupAddr}>{selectedItem.data.operator}</Text>
                    </View>
                  )}
                  {selectedItem.data.working_hours && (
                    <View style={styles.addressRow}>
                      <Ionicons name="time-outline" size={14} color="#9CA3AF" />
                      <Text style={styles.popupAddr}>{selectedItem.data.working_hours}</Text>
                    </View>
                  )}
                  {selectedItem.data.reward_description && (
                    <View style={styles.rewardBox}>
                      <MaterialCommunityIcons name="gift-outline" size={16} color="#B45309" />
                      <View style={{ flex: 1 }}>
                        {selectedItem.data.reward_type && (
                          <Text style={styles.rewardType}>
                            {REWARD_TYPE_LABELS[selectedItem.data.reward_type] ?? selectedItem.data.reward_type}
                          </Text>
                        )}
                        <Text style={styles.rewardDesc}>{selectedItem.data.reward_description}</Text>
                      </View>
                    </View>
                  )}
                  {(selectedItem.data.verification_count ?? 0) > 0 && (
                ...
                >
                  {verifying ? (
                    ...
                    <>
                      ...
                      <Text style={styles.verifyButtonText}>Tasdiqlash (+5 ball)</Text>
                    </>
                  )}
                </TouchableOpacity>
              )}
              ...
              {!user && (
                <TouchableOpacity
                  ...
                  onPress={() => { closeBottomSheet(); router.push("/auth"); }}
                >
                  <Ionicons name="log-in-outline" size={15} color={C.primary} />
                  <Text style={styles.loginPromptText}>Tasdiqlash uchun tizimga kiring</Text>
                </TouchableOpacity>
              )}
            </>
          )}
...
const styles = StyleSheet.create({
  ...
  bottomSheetContent: { padding: 16, gap: 10, paddingBottom: 32 },
  popupBadgeRow: { flexDirection: "row", alignItems: "center", gap: 8, flexWrap: "wrap" },
  badge: {
    flexDirection: "row",
    alignItems: "center",
  ...
  badgeText: { fontFamily: "Nunito_600SemiBold", fontSize: 12 },
  popupTitle: { fontFamily: "Nunito_700Bold", fontSize: 17, color: "#111" },
  popupSub: { fontFamily: "Nunito_400Regular", fontSize: 13, color: "#6B7280" },
  addressRow: { flexDirection: "row", alignItems: "flex-start", gap: 4 },
  popupAddr: { fontFamily: "Nunito_400Regular", fontSize: 12, color: "#9CA3AF", flex: 1 },
  confirmRow: {
    flexDirection: "row",
    alignItems: "center",
    gap: 6,
  ...
  confirmText: { fontFamily: "Nunito_600SemiBold", fontSize: 13, color: "#16A34A" },
  rewardBox: {
    flexDirection: "row",
  ...
  rewardType: { fontFamily: "Nunito_700Bold", fontSize: 12, color: "#B45309" },
  rewardDesc: { fontFamily: "Nunito_600SemiBold", fontSize: 13, color: "#92400E", marginTop: 2 },
  ...
  closeButtonText: { fontFamily: "Nunito_600SemiBold", fontSize: 14, color: "#374151" },
  directionsButton: {
    flex: 1,
    backgroundColor: C.primary,
    borderRadius: 12,
  ...
  directionsButtonText: { fontFamily: "Nunito_600SemiBold", fontSize: 14, color: "#fff" },
  verifyButton: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
  ...
  verifyButtonText: { fontFamily: "Nunito_700Bold", fontSize: 14, color: "#fff" },
  loginPrompt: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
  ...
  loginPromptText: { fontFamily: "Nunito_600SemiBold", fontSize: 13, color: C.primary },
});
